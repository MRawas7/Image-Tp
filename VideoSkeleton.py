...
class VideoSkeleton:
    ...
    def draw_reduced(self, skeleton):
        # Create a blank image
        height, width = 256, 256  # or any other size
        skeleton_image = np.zeros((height, width, 3), dtype=np.uint8)

        # Assuming 'ske' is the skeleton data you want to visualize
        # You may need to define these parameters according to your context
        ske = skeleton  # or however you are storing your skeleton data
        w, h = width, height  # Image dimensions
        idx = 0  # This could be an index related to your skeleton frames or similar

        # Call the joint method with the required arguments
        for joint in skeleton.joint(ske, w, h, idx):  # Provide the required arguments
            x, y = int(joint[0]), int(joint[1])  # Assuming joint returns (x, y)
            cv2.circle(skeleton_image, (x, y), 5, (0, 255, 0), -1)  # Draw joints

        # Optionally, draw lines between joints if your skeleton structure allows
        joints_list = skeleton.joint(ske, w, h, idx)  # Get the joints list
        for i in range(len(joints_list) - 1):
            cv2.line(skeleton_image, (int(joints_list[i][0]), int(joints_list[i][1])),
                     (int(joints_list[i + 1][0]), int(joints_list[i + 1][1])), (255, 0, 0), 2)
